        ...
        for (i = 0; i < POP_SZ; ++i)
            ...
            for (j = i + 1; j < POP_SZ; ++j)
                ...
                if (pdoc.GetRandom1st())
                    ...
                    {
                    strati = pop[i] & 1UL;  // grab turn 1 strategy
                    stratj = pop[j] & 1UL;
                    }
                ...
                for (k = 0; k < l; ++k)
                    {
                    // select my move
		    // "bit" will be a mask that selects your next play from your strategy string (chromosome).
		    // It is selected based on your opponent's prior play(s).  The sequence or
		    // pattern of plays your opponent makes will drive what your play is.
		    // "shift[k]" is a simple index to the start of your strategy string that
		    // deals with your opponent's prior plays.  See page 114 in the book.
		    // We add your opponent's prior plays (strati/j) to this index to find
		    // your response to that sequence of plays.
		    // The logical OR to create tempi/j just adds your new play to the record of
		    // all your plays thus far against that opponent.
